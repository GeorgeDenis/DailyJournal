...
@PageTitle("Fitness delete")
@Route(value = "fitness-delete", layout = MenuLayout.class)
@Uses(Icon.class)
public class FitnessDeleteForm extends Div {

    private NumberField id = new NumberField("Fitness entry id");
    private Button cancel = new Button("Cancel");
    private Button save = new Button("Save");

    private final UserService userService;

    public FitnessDeleteForm(FitnessService fitnessService, UserService userService) {
        this.userService = userService;
        addClassName("deletefitness-form-view");
        ...
        save.addClickListener(e -> {
            ...
            if(userLogged == null){
                ...
                if (idValue != null) {
                    int fitnessId = idValue.intValue();
                    ResponseEntity<String> response = fitnessService.deleteFitness(fitnessId);

                    if (response.getStatusCode() == HttpStatus.CREATED) {
                        Notification.show("Fitness entry deleted successfully.");
    ...
    private Component createTitle() {
        return new H3("Delete a fitness entry");
    }
